...
def standardize_mhc(mhc):
    ...
    if 'Mamu' not in mhc and 'Patr' not in mhc:
        mhc = mhc.upper()
    mhc = mhc.replace('*', '')
    mhc = mhc.replace(':', '')
    mhc = mhc.replace('-', '')
...
def mask_peptides(peptides, pad_aa='Z', max_len=11):
    '''
...
def cut_pad_peptides(peptides, max_len=9, padding_aa='X', padding_pos=6):
    '''
...
class Dataset():
    ...
    def mask_peptides(self, pad_aa='Z', max_len=11):
        '''
        Mask and pad for an LSTM
        '''
